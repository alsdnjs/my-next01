    ...
    script.onload = () => {
      if (window.kakao && window.kakao.maps) {
        window.kakao.maps.load(() => {
          ...
          const mapOptions = {
            center: new window.kakao.maps.LatLng(37.3241008, 126.9780), // 서울의 좌표
  ...
  >
      ...
      <div style={{ width: "65%" }}>
        <div style={{ textAlign: "left" }}>
          <h3 style={{ marginTop: "3px" }}>캠핑장 수: {filteredCamps.length}</h3>
        </div>
        <div
          style={{
            display: "flex",
            ...
            getCurrentPageData().map((camp) => (
              ...
              <div
                ...
                style={{
                  ...
                  display: "flex",
                  flexDirection: "column",
                  alignItems: "flex-start", // 'left' 대신 'flex-start'
                  borderBottom: "1px solid #ddd", // 경계선 추가
                  width: "40%", // 경계선의 길이 조정, 화면 왼쪽에 붙기
                }}
              >
                <strong onClick={() => handleCampClick(camp)}>{camp.facltNm}</strong>
                <div onClick={() => handleCampClick(camp)}>{camp.addr1}</div>
                <div>{camp.tel}</div>
              </div>
            ))
          ) : (
